...
class ColorPicker(QObject):
    ...
    def setSourceSize(self, ww: int, hh: int):
        self.ww = ww
        self.hh = hh
    ...
    def Analyse( self, qimage: QImage ):
        if ( self.pigment_o != None and qimage.isNull() == False ):
            report = self.pigment_o.API_Image_Analyse( qimage )
            return f"{ report }"
        else:
            return "Pigment.O not present"

    def Event( self, ex: int, ey: int, qimage_grab: QImage, state_press: bool, mode_pickcolor: bool ):
        if ( mode_pickcolor == True and qimage_grab != None ):
            # Event
            ex = Limit_Range( ex, 0, self.ww - 1 )
            ey = Limit_Range( ey, 0, self.hh - 1 )
            # Picker Location
            pos = QPoint( ex, ey )
            point = self.source_widget.mapToGlobal( pos )
    ...
    def Render( self, painter: QPainter, ex: int, ey: int ):
        # Values
