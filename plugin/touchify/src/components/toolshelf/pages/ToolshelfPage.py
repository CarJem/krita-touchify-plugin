...
class ToolshelfPage(QWidget):
    ...
    def _createSection(self, actionInfo: CfgToolshelfDocker):
        actionWidget: DockerContainer | ToolshelfQuickActions = None
        if actionInfo.section_type == "docker":
            actionWidget = DockerContainer(self, actionInfo.id, self.docker_manager)
            #region DockerContainer Setup``
    ...
    def _insertSections(self, widget_groups: Mapping[int, Mapping[int, list[DockerContainer | ToolshelfQuickActions]]]):
