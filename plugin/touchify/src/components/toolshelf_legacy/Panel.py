...
class Panel(QWidget):
    ...
    def Data_Recieved(self, widget_groups: WIDGET_GROUP):
        ...
        def Init_Cell(x: int, y: int, splitter: Panel.SectionSplit, sections: list[ToolshelfDataSection]):
            if len(sections) == 1:
                ...
                for section in sections:
                    ...
                    if isinstance(item, DockerContainer): tabBar.addTab(item, self.managers.mgr_dockers.dockerWindowTitle(item.docker_id))
        ...
        def Section_Docker(actionInfo: ToolshelfDataSection):
            actionWidget = DockerContainer(self, actionInfo.docker_id, self.managers.mgr_dockers)
