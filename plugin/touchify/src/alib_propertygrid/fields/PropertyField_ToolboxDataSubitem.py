...
from jemlib.api_touchify.config.toolbox.ToolboxDataItem import ToolboxDataSubitem
...
class PropertyField_ToolboxDataSubitem(PropertyField_TypedList):
    def __init__(self, handler: "DataHandler", property: DataPath[TypedList[ToolboxDataSubitem]]):
        manual_restrictions = [DataConstraints.listMod(DataConstraints.ListMod.Icons)]
        super(PropertyField_ToolboxDataSubitem, self).__init__(handler, property, manual_restrictions)

    def list_on_quick_add(self, source: str):
        new_item = ToolboxDataSubitem()
