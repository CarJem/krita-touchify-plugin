...
from touchify.src.api_krita.enums import Action, Tool, Toggle, TransformMode
from touchify.src.api_krita.actions import TransformModeFinder
from touchify_pie_wheels.src.composer_utils.label import LabelText, LabelTextColorizer
from touchify_pie_wheels.src.core_components.controller_base import Controller, NumericController
...
class ToolController(Controller[Tool]):
    ...
    def get_value() -> Tool:
        ...
        return KritaAPI.active_tool

    @staticmethod
    def set_value(value: Tool) -> None:
        """Set a passed tool."""
        KritaAPI.active_tool = value
...
class UndoController(NumericController):
    ...
    def set_value(self, value: float) -> None:
        ...
        if round(value) == self.state:
            ...
            KritaAPI.trigger_action("edit_redo")
            self.state += 1
        else:
